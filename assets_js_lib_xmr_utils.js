...
function get_ssk(bip39, seed) {
    let p_rootkey = (seed === true) ? get_rootkey(bip39) : get_rootkey(toseed(bip39)),
        dx_dat = {
...
function xmr_getpubs(ssk, index) {
    ...
    let pubp = point_multiply(sc_reduce32(fasthash(5375624164647200 + svk + uint32hex(0) + uint32hex(index)))),
...
function strtobin(str) {
    let res = uint_8Array(str.length);
    for (let i = 0; i < str.length; i++) {
...
function base58_encode(data) {
    let ab = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz",
        ab_map = {},
...
function check_pid(payment_id) {
    ...
    let pattern = RegExp("^[0-9a-fA-F]{16}$");
