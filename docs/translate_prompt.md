...
1. **Do NOT translate placeholders.** Placeholders like `{currency}`, `{amount}`, `{url}`, `{address}`, etc. are filled in by the app and must remain exactly as they are, curly braces included. You may rearrange their position to fit natural sentence structure in [YOUR LANGUAGE].
...
4. **Rename the function and object** from `lang_en` / `lang_en_obj` to `lang_[COUNTRYCODE]` / `lang_[COUNTRYCODE]_obj`. If the code contains a hyphen, replace it with an underscore (e.g. `zh-cn` → `lang_zh_cn`).
