<?php
	...
	const ALLOWED_SHARE_ORIGINS = ["https://bitrequest.github.io/", "https://bitrequest.web.app/"];

	// The stored payload is base64(json{sharedurl,sitethumb}); only accept share
	// urls that point back at the PWA so the /inv/4bR redirect can't be abused.
	function valid_longurl($b64) {
		$decoded = base64_decode($b64, true);
		if ($decoded === false) return false;
		$content = json_decode($decoded, true);
		if (!is_array($content) || empty($content["sharedurl"]) || !is_string($content["sharedurl"])) return false;
		foreach (ALLOWED_SHARE_ORIGINS as $origin) {
			if (strpos($content["sharedurl"], $origin) === 0) return true;
		}
		return false;
	}
	...
	function handle_post($data) {
		...
		if (!valid_longurl($long_url)) {
			send_response(["error" => "Invalid longurl"]);
			return;
		}
