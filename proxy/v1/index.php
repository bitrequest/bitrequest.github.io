...
function handle_fetchcreds_request($postdata, $keys) {
    ...
    // No cache folder: token responses must never be written to disk.
    return api("https://oauth2.googleapis.com/token", $postdata, null, 0, null, null, null);
}
...
try {
    ...
    // Re-validate after key_param/ampersand were appended: they are caller-controlled
    // and must not move the request to a host the server key isn't allowed for.
    if ($server_key && $key_param2 !== "" && !api_host_allowed($apiname, $new_url)) {
        echo json_encode(["ping" => create_error_response("403", "Host not allowed for API key")]);
        exit;
    }
