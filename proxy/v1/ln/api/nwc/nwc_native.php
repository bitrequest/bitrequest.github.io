...
// NIP-04 (unauthenticated AES-CBC) is deprecated in NIP-47. This fallback now
// defaults to NIP-44 (see nwc_build_event) and keeps NIP-04 only to (a) read
// responses from wallets that still reply in it and (b) allow an explicit nip04
// override for a legacy wallet that can't do NIP-44.
...
// Decrypt a response by sniffing the scheme: NIP-04 payloads carry a "?iv="
// separator, NIP-44 payloads don't. Reads whatever the wallet replied with.
function nwc_decrypt_content($content, $privkey_hex, $peer_xonly_hex) {
    return strpos($content, "?iv=") !== false
        ? nwc_nip04_decrypt($content, $privkey_hex, $peer_xonly_hex)
        : nip44_decrypt($content, $privkey_hex, $peer_xonly_hex);
}
...
// encryption: "nip44" (default) | "nip04". NIP-44 adds the negotiated encryption
// tag per NIP-47; NIP-04 is the deprecated legacy path.
function nwc_build_event($method, $params, $privkey_hex, $wallet_pubkey_hex, $encryption = "nip44") {
    // Cast params to stdClass so empty params encode as `{}`, not `[]`.
    $content_plain = json_encode([
        "method" => $method,
        "params" => (object)$params
    ], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    $use_nip44 = $encryption !== "nip04";
    $content = $use_nip44
        ? nip44_encrypt($content_plain, $privkey_hex, $wallet_pubkey_hex)
        : nwc_nip04_encrypt($content_plain, $privkey_hex, $wallet_pubkey_hex);
    $pubkey = secp256k1_xonly_pubkey($privkey_hex);
    $created_at = time();
    $kind = 23194;
    $tags = $use_nip44
        ? [["p", $wallet_pubkey_hex], ["encryption", "nip44_v2"]]
        : [["p", $wallet_pubkey_hex]];
 ...
 */
function nwc_request_native($nwc_uri, $method, array $params, $encryption = "nip44") {
    ...
    $event = nwc_build_event($method, $params, strtolower($secret), $wallet_pubkey, $encryption);
    ...
    try {
        ...
        while (microtime(true) < $deadline) {
            ...
            if ($msg[0] === "EVENT" && isset($msg[2]) && ($msg[2]["kind"] ?? 0) === 23195) {
                $plain = nwc_decrypt_content($msg[2]["content"], strtolower($secret), $wallet_pubkey);
