...
function api($url, $data, $headers, $ct, $cfd, $meta, $fn) {
	...
	if ($fn) {
		...
		$filename = md5((is_string($data) ? $data : json_encode($data)) . $url);
	}
...
// Removes credential-bearing headers from the flat "Key: Value" header array so
// they aren't re-sent across a cross-host redirect. Names matched case-insensitively.
function strip_credential_headers($headers) {
	if (empty($headers)) {
		return $headers;
	}
	static $sensitive = ["authorization", "grpc-metadata-macaroon", "rune", "x-api-key"];
	$filtered = [];
	foreach ($headers as $header) {
		$name = strtolower(trim(strstr($header, ":", true)));
...
function curl_get($url, $data, $headers) {
	try {
		...
		$current_url = $url;
		$max_redirects = 3;
		$origin_host = strtolower(parse_url($url, PHP_URL_HOST) ?? "");
