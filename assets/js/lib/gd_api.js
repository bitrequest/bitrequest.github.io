...
function set_auth_active() {
    ...
    if (stored_token) {
        ...
        br_set_local("dat", stored_token, true);
    }
}
...
function start_google_auth() {
    ...
    const consent_param = auth_state.expired == "norefresh" ? "&prompt=consent" : "",
        oauth_state = sha_sub(String(now_utc()) + generate_random_number(0, 4294967295), 32), // CSRF token, verified on return
        auth_url = "https://accounts.google.com/o/oauth2/auth?client_id=" + to.ga_id + "&redirect_uri=" + glob_const.redirect_uri + "&response_type=code&scope=" + glob_const.scope + "&access_type=offline&state=" + oauth_state + consent_param;
    br_set_session("oauth_state", oauth_state);
    glob_const.w_loc.href = auth_url;
}

// Only accept an authorization code that answers a login we started in this session
function valid_oauth_state(state) {
    const expected = br_get_session("oauth_state");
    br_remove_session("oauth_state");
...
function create_drive_file(access_token) {
    const token_data = get_auth_status(),
        ...
        backup_content = compile_backup_data();
    if (final_token) {
...
function delete_drive_file(file_id, list_item, access_token, callback) {
    api_proxy({
