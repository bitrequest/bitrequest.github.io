...
function derive_x(derive_params, from_private) {
    ...
    for (let i = 0; i < path_segments.length; i++) {
        ...
        if (i > 0) {
            const segment_match = /^(\d+)(')?$/.exec(segment);
            if (!segment_match) {
                throw new Error("Invalid derivation path segment '" + segment + "' in " + path);
            }
            const is_hardened = segment_match[2] === "'";
            if (is_hardened && is_public) {
                throw new Error("Hardened derivation is impossible from a public key: " + path);
            }
            const index_num = Number(segment_match[1]);
            if (index_num >= 2147483648) {
                throw new Error("Derivation index out of range: " + segment);
            }
            const child_index = is_hardened ? dec_to_hex(index_num + 2147483648) : str_pad(dec_to_hex(index_num), 8),
...
function keypair_array(seed, indices, start_index, derive_path, bip32_config, key, chain_code, coin, version, formatter) {
    const format_fn = formatter || format_keys,
        derived_pairs = [];
    for (let i = 0; i < indices.length; i++) {
        const current_index = i + start_index,
            ...
            formatted_keys = format_fn(seed, ext_keys_result, bip32_config, current_index, coin);
...
function xpub_obj(coin, root_path, chain_code, key, fingerprint = "00000000", childnumber = "80000000") {
