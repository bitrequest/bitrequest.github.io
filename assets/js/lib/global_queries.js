...
const br_bipobj = br_get_local("bpdat", true),
     ...
     *
     * Organized by category. New keys should be added to the most appropriate
     * section. If a key doesn't fit any section, add it under "Misc" and consider
    ...
    glob_const = {
        ...
        "bchexplorer_agent": "BitrequestPWA/1.0",

        // --- Lightning ---
        "ln_socket": "wss://bitrequest.app:8030",
        "spark_host": "https://api.lightspark.com", // spark host placeholder

        // --- Misc external endpoints ---
        ...
        "post_scan_timeout": 30000, // Perform extra tx lookup when closing paymentdialog after 30 seconds
        "zeroplaceholder": parseFloat((0.00).toLocaleString(langcode, {
            "minimumFractionDigits": 2,
            "maximumFractionDigits": 2
        })).toFixed(2),

        // --- Test fixtures (used by unit_tests/ and the in-app diagnostics) ---
    ...
    /**
     * Runtime state. Mutated freely throughout the app's lifetime.
     *
     * Each property is tagged with its owning file — the file that primarily
     * reads/writes it. If you're tracking down "where is this set?", start there.
     * Properties without an owner tag are shared between several files.
     */
    glob_let = {
        // --- Scanning / payment monitoring ---
        "post_scan": false, // owner: polling.js  — true while a post-close re-scan is active
        "tpto": 0, // owner: polling.js  — transaction-polling setTimeout id
        "block_scan": 0, // owner: monitors.js — block-height scan counter
        "tx_count": 1000000, // owner: monitors.js — high-water mark for tx pagination
        "tx_list": [], // owner: monitors.js — accumulated tx hashes seen this scan
        "pinging": {}, // owner: polling.js  — { [address]: setInterval id } per address being polled
        "rpc_attempts": {}, // owner: monitors.js — fingerprints of in-flight rpc calls (de-dup)
        "api_attempt": {}, // owner: monitors.js — fingerprints of in-flight api calls
        "apikey_fails": false, // owner: monitors.js — last apikey attempt failed
        "proxy_attempts": {}, // owner: global_queries.js (api_proxy)
        "test_rpc_call": null, // owner: rpcs.js — last rpc test result
        // Retry budgets formerly lived in glob_let.block_overflow / .overflow_detected.
        // They are now per-request on rdo.attempts / request.attempts — see over_budget()
        // and over_budget_request() in this file.
        ...
        // --- Monero-specific scan state ---
        "xmr_indexed": { // owner: polling.js  — per-session "already-checked" sets
            "mempool": [], //   tx hashes seen in the mempool
            "blocks": [] //   block heights already scanned
        },

        // --- Ethereum L2 ---
        "l2_fetched": {}, // owner: ethl2.js    — { id, l2 } for last fetched L2 chain
        "l2s": {}, // owner: ethl2.js    — per-request L2 chain memo
        "is_erc20t": null, // owner: payments.js — flag: current dialog is an ERC20 token

        // --- BIP39 / seed state ---
        "bipid": br_bipid, // owner: bip39.js    — current seed id
        "bipobj": br_bipobj, // owner: bip39.js    — current encrypted seed blob
        "bipv": false, // owner: bip39.js    — seed has been verified this session
        "hasbip": br_hasbip, // owner: bip39.js    — a seed exists in storage
        "phrasearray": null, // owner: bip39.js    — in-progress mnemonic during setup
        "phraseverified": false, // owner: bip39.js
        "changes": {}, // owner: bip39.js    — unsaved seed edits
        "test_derive": true, // owner: bip39.js    — run derivation self-tests on next op

        // --- Backup / restore ---
        "backup_active": false, // owner: settings.js — backup flow in progress
        "backup_filename": null, // owner: settings.js — chosen backup file name
        "backup_result": null, // owner: settings.js — last backup attempt result

        // --- Cashier (PIN / shared-access) ---
        "cashier_dat": br_cashier_dat, // owner: core.js     — cashier mode config blob
        "cashier_seedid": br_cashier_seedid, // owner: core.js
        "is_cashier": br_is_cashier, // owner: core.js     — running in cashier mode
        "ap_id": null, // owner: core.js     — active passcode id

        // --- Lightning / NFC ---
        "lnd_confirm": false, // owner: lightning.js
        "lnd_ph": false, // owner: lightning.js — placeholder invoice active
        "ndef_processing": null, // owner: sockets.js   — NFC NDEFReader in-flight read id
        "ndef_timer": 0, // owner: sockets.js   — NFC retry timer
        ...
        "scantype": null, // owner: core.js      — qr-scanner mode flag
        "currencyscan": null, // owner: coin_settings.js — currency picker scan state
        "hascam": false, // owner: core.js      — camera detected
        "sa_timer": 0, // owner: payments.js  — share-amount input debounce timer
        "cp_timer": 0, // owner: payments.js  — currency-pool refresh debounce
        "request_timer": 0, // owner: payments.js  — request-dialog idle timer
        "statuspush": [], // owner: monitors.js  — queued status updates for currently-rendering UI
        "wakelock": false, // owner: sockets.js   — screen wake-lock sentinel
        "new_address": false, // owner: bip39.js     — guard against double address inserts
        "progress": 0, // owner: bip39.js     — derivation progress %
        "percent": 0, // owner: bip39.js     — derivation progress %
        "audio_ctx": null, // owner: global_queries.js — Web Audio context, lazy-init

        // --- Storage / boot ---
        "init": br_init, // owner: core.js      — first-run init blob from localStorage
        "io": br_io, // owner: core.js      — initial-options object
        "local": false, // owner: core.js      — running on localhost without php
        "localserver": false, // owner: core.js      — running on localhost WITH php
        "is_btc": false, // owner: payments.js  — current dialog is BTC-family
        "resd": {}, // owner: core.js      — last response data (debugging)
        "symbolcache": false, // owner: core.js      — fiat symbol cache populated
...
function br_set_local(pref, dat, str) {
    if (dat === undefined) {
        console.warn("br_set_local: refusing to store undefined for key '" + pref + "'");
        return
    }
    const storage_data = str ? JSON.stringify(dat) : String(dat);
...
function br_set_session(pref, dat, str) {
    if (dat === undefined) {
        console.warn("br_set_session: refusing to store undefined for key '" + pref + "'");
        return
    }
    const storage_data = str ? JSON.stringify(dat) : String(dat);
...
function fulldateformat(date, lang, markup) {
    const year = date.getFullYear(),
        ...
        weekday = (lang !== langcode) ? date.toLocaleString(lang, {
            weekday: "long"
...
// Registers a polling interval under a key, clearing any interval already registered under it
function set_ping(ping_id, fn, interval) {
    if (glob_let.pinging[ping_id]) {
        clearInterval(glob_let.pinging[ping_id]);
    }
    glob_let.pinging[ping_id] = setInterval(fn, interval);
}

// Exact address comparison, tolerant of known prefixes (0x, bitcoincash:)
function norm_addr(address) {
    return String(address).trim().toLowerCase().replace(/^(bitcoincash:|0x)/, "");
}

function addr_eq(addr1, addr2) {
    if (!addr1 || !addr2) return false
    return norm_addr(addr1) === norm_addr(addr2);
}
...
function safe_file_part(str) {
    return String(str)
        .replace(/[\/\\:*?"<>|,]/g, "_") // illegal chars AND separators → underscore
        .replace(/\s+/g, "_")
        .replace(/_+/g, "_")
        .replace(/^_+|_+$/g, "");
}
...
function init_audio_context() {
    ...
    load_audio("funk", "assets/sounds/funk.mp3");
    load_audio("cashier", "assets/sounds/cashier.mp3");
    load_audio("collect", "assets/sounds/collect.mp3");
    load_audio("blip", "assets/sounds/blip.mp3");
    load_audio("waterdrop", "assets/sounds/waterdrop.mp3");
    load_audio("howl", "assets/sounds/howl.mp3");
...
function get_aws_icon_url(wallet_name, clas = "wallet_icon", ext = "png") {
    return "<img src='" + glob_const.aws_bucket + "img_icons_wallet-icons_" + wallet_name + "." + ext + "' class='" + clas + "' onerror=\"this.src='assets/img/wp_holder.png'\">";
}
...
// ============================================================
// Overflow / retry-budget prevention
//
// Each request carries its own retry budget on `rdo.attempts` (or
// `request.attempts` for guards that fire outside a per-rdo context — proxy
// rotation, socket fallback, and the Monero setup path). Exceeding the
// budget aborts only the current request; other requests are unaffected.
// The counter dies when the request ends — no global reset semantics, no
// session-wide kill switch.
//
// Default budgets are conservative but generous. Callers can override the
// limit per call site (route_layer2_api_request does this — addr_polling
// runs much more often and gets 150).
// ============================================================

const RETRY_BUDGETS = {
    "api": 25, // try_next_api rotations
    "socket": 10, // socket fallbacks per request
    "rpc": 10, // L1 RPC rotations
    "l2": 25, // L2 API rotations
    "l2_init": 10, // L2 init path (route_layer2_api_request)
    "polling": 25, // tx_polling / l2_polling re-entry
...
function over_budget(rdo, kind, limit) {
    ...
    rdo.attempts = rdo.attempts || {};
    rdo.attempts[kind] = (rdo.attempts[kind] || 0) + 1;
    const max = limit || RETRY_BUDGETS[kind] || 25;
    if (rdo.attempts[kind] > max) {
        console.warn("over_budget:", kind, rdo.attempts[kind] + " > " + max, "for request", (rdo.request_id || rdo.source || "?"));
        return true
    }
    return false
}
...
function over_budget_request(kind, limit) {
    ...
    request.attempts[kind] = (request.attempts[kind] || 0) + 1;
    const max = limit || RETRY_BUDGETS[kind] || 25;
    if (request.attempts[kind] > max) {
        console.warn("over_budget_request:", kind, request.attempts[kind] + " > " + max);
        return true
    }
    return false
}
...
function check_params(params, app) {
    ...
    if (url_params.p === "settings") {
        if (url_params.code) {
            if (valid_oauth_state(url_params.state)) {
                validate_auth_state(url_params.code);
            } else {
                console.warn("oauth: state mismatch, ignoring authorization code");
                history.replaceState(null, "", glob_const.w_loc.pathname + "?p=settings");
            }
            return
        }
    }
...
function parse_url_params(str) {
    ...
    for (const pair of param_pairs) {
        ...
        const eq_index = pair.indexOf("="),
            raw_key = pair.slice(0, eq_index),
            raw_val = pair.slice(eq_index + 1);
        let decoded_key, decoded_val;
        try {
            decoded_key = decodeURIComponent(raw_key);
            decoded_val = raw_val ? decodeURIComponent(raw_val.replace(/\+/g, " ")) : "";
        } catch (e) {
            continue; // malformed percent-encoding: skip the pair instead of throwing on boot
        }
...
function inj(val) {
    ...
    const xss_pattern = /<\s*[\/]?\s*(script|img|svg|iframe|object|details|embed|style|math|link|video|audio|form|input|button|marquee|isindex|body|meta|base|applet|param|frameset|frame)|[\s"'\/]on\w+\s*=|javascript:|vbscript:|data:text\/(html|javascript)|expression\(|href\s*=\s*["']?\s*javascript:|src\s*=\s*["']?\s*(javascript:|data:)|formaction\s*=\s*["']?\s*javascript:|action\s*=\s*["']?\s*javascript:|style\s*=.*(?:expression|url\(javascript:)/i;
