...
function str_pad(val, bytes) {
    return (bytestring.slice(0, bytes) + val).slice(-bytes);
}
...
// 40-bit BCH polymod shared by the CashAddr and Kaspa checksum schemes
function cashaddr_polymod(values) {
    const GENERATORS = [
...
// Prefix expansion shared by CashAddr and Kaspa: low 5 bits of each char, then 0
function cashaddr_prefix_expand(prefix) {
    const result = [];
...
// Converts a 40-bit polymod result to 8 five-bit checksum values
function cashaddr_checksum_array(mod) {
    const checksum = new Array(8);
    for (let i = 0; i < 8; i++) {
...
// Kaspa polymod (numeric result, same generator polynomial as CashAddr)
function kaspa_polymod(data) {
    return Number(cashaddr_polymod(data));
}

// Create 8-character checksum for Kaspa addresses
function kaspa_create_checksum(prefix, data) {
    const checksumData = cashaddr_prefix_expand(prefix).concat(data).concat([0, 0, 0, 0, 0, 0, 0, 0]);
    return cashaddr_checksum_array(cashaddr_polymod(checksumData));
}
...
function pub_to_kaspa_address(pubkey) {
    ...
    for (let i = 0; i < x_only.length; i += 2) {
        pubkey_bytes.push(parseInt(x_only[i] + x_only[i + 1], 16));
    }
...
// aes_enc prepare
function secret_to_key(secret) {
    return sjcl.codec.base64.fromBits(sjcl.hash.sha256.hash(secret));
}

function aes_encode(data, secret) {
    const payload = typeof data === "string" ? data : JSON.stringify(data);
    return aes_enc(cleanb64(payload), secret_to_key(secret));
}

function aes_encode(data, secret) {
    const payload = typeof data === "string" ? data : JSON.stringify(data);
    return aes_enc(btoa(unescape(encodeURIComponent(payload))), secret_to_key(secret));
}

function aes_decode(encrypted, secret) {
    const decrypted = aes_dec(encrypted, secret_to_key(secret));
    if (!decrypted) return null;
    try {
        const raw = atob(strip_quotes(decrypted));
        let payload;
        try {
            payload = decodeURIComponent(escape(raw)); // new UTF-8 payloads
        } catch (e) {
            payload = raw; // legacy plain-btoa payloads
        }
        return JSON.parse(payload);
    } catch (e) {
        return decrypted; // not base64/JSON at all
    }
}
...
const cashaddr = (function() {
    ...
    function convertBits(data, fromBits, toBits, pad) {
        const result = convert_bits(data, fromBits, toBits, pad);
    ...
    function createChecksum(prefix, payload) {
        const combined = cashaddr_prefix_expand(prefix).concat(Array.from(payload), [0, 0, 0, 0, 0, 0, 0, 0]);
        return new Uint8Array(cashaddr_checksum_array(cashaddr_polymod(combined)));
    }

    function verifyChecksum(prefix, payload) {
        const combined = cashaddr_prefix_expand(prefix).concat(Array.from(payload));
        return cashaddr_polymod(combined) === 0n;
    }
