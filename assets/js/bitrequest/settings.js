    ...
    api_proxy({
        ...
        if (ping) {
            const cache = ping.br_cache,
                ...
                title = "Bitrequest " + tl("systembackup") + " " + account + " (" + time_format + ")",
        ...
        if (key === "bitrequest_bpdat") {
            if (newphrase) {
                if (glob_let.test_derive) {
                    if (bpdat) {
                        const seedobj = build_seedobject(bpdat.dat, bpdat.id, get_setting("pinsettings", "pinhash"));
                        if (seedobj) {
                            localStorage.setItem(key, JSON.stringify(seedobj));
                        }
                    }
                }
            }
...
function compile_teaminvite() {
    const jsonfile = {},
        skip_keys = [
            ...
            "bitrequest_backupfile_id", "bitrequest_appstore_dialog", "bitrequest_init",
            "bitrequest_eth_l2_contracts", "bitrequest_k", "bitrequest_awl", "bitrequest_dat", "bitrequest_rt", "bitrequest_tp", "bitrequest_bpdat",
            "bitrequest_requests", "bitrequest_archive"
        ];
    for (let key in localStorage) {
