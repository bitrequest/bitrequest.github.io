...
// Initiates BIP32 seedphrase management with PIN verification for existing seedphrases
function manage_bip32_seedphrase() {
    $(document).on("click", "#bip39_seedphrase", function() {
        if (glob_let.hasbip) {
            all_pinpanel({
                "func": manage_bip32
            }, null, true);
...
function share_backup_file() {
    $(document).on("click", "#share_bu", function() {
        ...
        if (result) {
            ...
            // PIN is NOT enabled → prevent sharing backup and show PIN panel
            all_pinpanel({
                "func": share_bu
            }, true, true);
        }
...
function share_bu() {
    ...
    api_proxy({
        ...
        if (ro) {
            ...
            shorten_url(title, app_root + "?sbu=" + data, app_root + "assets/img/icons/apple-touch-icon.png", true, url_hash);
        }
    ...
    api_proxy({
        ...
        if (ping) {
            const cache = ping.br_cache,
                ...
                account = b64decode_url(result.account) || "",
                safe_account = escape_html(account),
                title = "Bitrequest " + tl("systembackup") + " " + safe_account + " (" + time_format + ")",
                title_attr = "Bitrequest " + tl("systembackup") + " " + account + " (" + time_format + ")",
                bu_date = safe_file_part(time_format),
                cache_time = cache.cache_time,
                expires = (file_time + cache_time) - server_time,
                filename = "bitrequest_system_backup_" + safe_file_part(account) + "_" + langcode + "_" + bu_date + ".json",
                cd = countdown(expires * 1000),
                cd_format = countdown_format(cd),
                exp_class = cd_format ? "" : "expired",
                                            ...
                                            "content": [{
                                                ...
                                                "a": {
                                                    ...
                                                    "attr": {
                                                        "data-restore": "yes",
                                                        "href": "#",
                                                        "title": escape_attr(filename),
                                                        "data-date": escape_attr(bu_date),
                                                        "data-lastbackup": escape_attr(filename),
                                                        "download": escape_attr(filename),
                                                        "data-blob": cb64
                                                    },
                                                    "content": tl("downloadbu")
                                                },
                                                "div": {
                                                    ...
                                                    "attr": {
                                                        "title": tl("install") + " " + title_attr,
                                                        "data-base64": cb64,
                                                        "data-filename": escape_attr(filename)
                                                    },
...
function compile_backup_data() {
    const bu_dat = {},
        skip_keys = [
            ...
            "bitrequest_eth_l2_contracts", "bitrequest_k", "bitrequest_awl", "bitrequest_dat", "bitrequest_rt", "bitrequest_tp", "bitrequest_bpdat", "sameseed"
        ];
    for (let key in localStorage) {
        ...
        // Try to keep the original type if it’s JSON, otherwise store as string
        try {
            bu_dat[key] = JSON.parse(value);
        } catch (e) {
            bu_dat[key] = value;
        }
    }
    const phrase_obj = ls_phrase_obj(),
        pin_hash = get_setting("pinsettings", "pinhash"),
        lock_id = phrase_obj ? "b39" : pin_hash ? "pn" : "rd",
        r_pick = Math.floor(Math.random() * 9) + 1,
        random_salt = Object.values(crypto_utils_const)[r_pick],
        secret = (lock_id === "b39") ? JSON.stringify(phrase_obj.pob) : (lock_id === "pn") ? pin_to_encryption_key(pin_hash, random_salt) : random_salt,
        lock_trail = (lock_id === "b39") ? phrase_obj.pid : r_pick + String(now_utc()).slice(-3),
        lock = lock_id + lock_trail,
        encr = aes_encode(bu_dat, secret),
        json = {
            lock,
            encr
        },
...
function generate_backup_filename() {
    return "bitrequest_backup_" + langcode + "_" + now_filename_stamp() + ".json";
}
...
function submit_backup() {
    $(document).on("click", "#triggerdownload", function(e) {
        ...
        if (!result) return
        // PIN is enabled → allow the download to continue
        if (check_pin_enabled(true) === true || node.attr("data-restore")) { // no need to set pin on restore download
            setTimeout(() => backup({
                node
            }), 100);
            return
        }
        // PIN is NOT enabled → show PIN panel
        all_pinpanel({
            "func": backup,
            "args": {
                node
...
function backup(args) {
    const node = args.node,
        last_saved = "last backup: " + node.attr("data-date"),
        last_backup = node.attr("data-lastbackup"),
        restore = node.attr("data-restore");
    if (!restore) {
        set_setting("backup", {
...
function submit_restore() {
    $(document).on("click", "#restoreformbox input.submit", function(e) {
        e.preventDefault();
        const gd_backuplist = $("#gd_backuplist");
        if (gd_backuplist.is(":visible")) {
            const gd_selected = $("#gd_backuplist li.selected");
            if (gd_selected.length) {
                gd_restore(gd_selected);
                return
            }
        ...
        if (glob_let.backup_active === true) {
            if (glob_let.backup_result) {
                const backup_result = glob_let.backup_result,
                    backup_data = backup_result.slice(backup_result.indexOf(",") + 1),
                    cb64 = strip_quotes(backup_data),
                    meta = {
                        "type": "file",
...
function gd_restore(field) {
    const file = field.text(),
...
// Select Google Drive backup file
function select_gd_restore() {
    $(document).on("click", "#gd_backuplist .restorefile", function() {
        const gd_backuplist = $("#gd_backuplist li"),
            field = $(this).parent("li");
        if (field.hasClass("selected")) {
            field.removeClass("selected");
            return
        }
        field.addClass("selected");
        gd_backuplist.not(field).removeClass("selected");
    });
}

// Validates and executes backup restoration with team invite handling
function restore(cb64, meta) {
    if (inj(cb64)) return
    if (inj(meta)) return
    const file = meta.filename,
        device = meta.thisdevice || "this",
        type_result = (meta.type === "file") ? tl("restorefile", {
            file
        }) : tl("restorefromdevice", {
            file,
            device
        }),
        result = confirm(type_result);
    if (result) {
        const json = JSON.parse(cb64),
            json_decrypt = json.lock ? decrypt_json_bu(json, meta) : json;
        if (json_decrypt) {
            if (json_decrypt === "bail") {
                return
            }
            continue_restore(json_decrypt, meta);
            return
        }
        popnotify("error", tl("error") + " " + tl("restorefrombackup"));
    }
}

// Continue restore
function continue_restore(json, meta) {
    if (!check_backup(json)) {
        return
    }
    if (is_team_invite(json)) {
        install_teaminvite(json, meta.filename, false);
        return
    }
    // no need to verify seedphrase from backup again
    const init_data = br_get_local("init", true),
        updated_init = get_default_object(init_data, true);
    updated_init.bipv = "yes";
    json.bitrequest_init = updated_init;
    meta.jasobj = json;
    restore_algo(meta);
}

// Decrypt json backup
function decrypt_json_bu(json, meta) {
    const lock = json.lock,
        type = (lock.startsWith("b39")) ? "b39" : (lock.startsWith("pn")) ? "pn" : "rd",
        filename = meta.filename;
    let secr;
    if (type === "b39") {
        const seedid = lock.slice(3),
            current_seed_id = glob_let.bipid,
            phrase_obj = ls_phrase_obj();
        if ((seedid === current_seed_id) && phrase_obj) {
            secr = JSON.stringify(phrase_obj.pob);
        } else {
            const dat = json.encr;
            if (dat) {
                bip39({
        ...
        const current_pin_hash = get_setting("pinsettings", "pinhash");
        if (current_pin_hash) {
            const r_pick = lock[2],
                random_salt = Object.values(crypto_utils_const)[r_pick];
            secr = pin_to_encryption_key(current_pin_hash, random_salt);
        }
    } else if (type === "rd") {
        const r_pick = lock[2];
        secr = Object.values(crypto_utils_const)[r_pick];
    }
    if (secr) {
        const restored = aes_decode(json.encr, secr);
        if (restored) {
            restored.sameseed = true;
            return restored;
        }
    }
    if (type === "pn") {
        glob_let.resd.pcnt = 0;
        json.meta = meta; // add meta data
        pin_dialog(json, filename);
        return "bail";
    }
    return false
}
...
function restore_algo(meta) {
    glob_let.resd = {
        "pcnt": 0
    };
    const data = meta.jasobj;
    if (data) {
        if (data.sameseed) {
            restore_callback(meta);
            return
        }
        const bpdat = data.bitrequest_bpdat;
        if (bpdat) { // seedphrase encoded backup
            const datenc = bpdat.datenc || bpdat,
                can_dec = s_decode(datenc);
            if (can_dec) {
                ...
                restore_callback(meta, true);
                return
            }
            pin_dialog(meta, "restore_callback");
            return
        }
        restore_callback(meta);
        return
...
function restore_callback(data, ns) {
    const type = data.type,
        newseed = ns || !glob_let.hasbip;
    if (newseed) {
        br_set_local("tp", now_utc());
    }
    if (type) {
        if (type === "gd") {
            restore_callback_gd(data);
            return
        }
        if (type === "file") {
            restore_callback_file(data);
            return
        }
    }
}
...
function restore_callback_gd(data) {
    restore_storage(data.jasobj);
    render_settings(["restore", "backup", "pinsettings"]);
    const restore_time = "last restore: " + now_display_stamp();
    set_setting("restore", {
    ...
    if (data.thisdeviceid === glob_const.deviceid) {
        const p = get_auth_status();
        if (p.pass) {
            delete_drive_file(data.thisfileid, null, p.token, "restore");
            return
        }
    }
    reset_changes();
    glob_const.w_loc.href = glob_const.w_loc.pathname;
}

// Executes file-based backup restoration and updates restoration history
function restore_callback_file(data) {
    restore_storage(data.jasobj);
    render_settings(["restore", "backup", "pinsettings"]);
    const restore_time = "last restore: " + now_display_stamp();
    set_setting("restore", {
...
function submit_pin_dialog() {
    $(document).on("click", "#pindialog input.submit", function(e) {
        ...
        const dialog = $("#dialog"),
            ...
            json = pass_dat.jasobj || pass_dat;
        if (!json) {
            return
        }
        const lock = json.lock;
        if (lock) { // decode pinlocked backup
            const pin_length = value.length;
            if (pin_length === 4) {
                const pin_hash = generate_hash(value),
                    r_pick = lock[2],
                    random_salt = Object.values(crypto_utils_const)[r_pick],
                    secr = pin_to_encryption_key(pin_hash, random_salt),
                    restored = aes_decode(json.encr, secr);
                if (restored) {
                    const meta = json.meta;
                    continue_restore(restored, meta);
                    return
                }
            }
...
function cs_callback() {
    const bpdat = glob_let?.resd?.bpdat;
    if (bpdat) {
        const current = ls_phrase_obj(),
            backup = ls_phrase_obj_parsed(bpdat),
            compare = {
...
function restore_storage(jsonobject) {
...
function csvexport_trigger() {
    $(document).on("click", "#csvexport", function() {
        ...
        if (has_requests || has_archive) {
            const filename = "bitrequest_csv_export_" + now_filename_stamp() + ".csv",
...
// Encrypts a payload with a random-salt (rd) lock for proxy sharing
function encode_rd_payload(data) {
    const r_pick = Math.floor(Math.random() * 9) + 1,
        random_salt = Object.values(crypto_utils_const)[r_pick],
        lock_trail = r_pick + String(now_utc()).slice(-3),
        lock = "rd" + lock_trail,
        encr = aes_encode(data, random_salt);
...
// Decrypts an rd-locked shared payload (team invites, shared CSV exports)
function decrypt_rd_payload(json) {
    const lock = json.lock,
        r_pick = lock[2],
        secr = Object.values(crypto_utils_const)[r_pick],
        restored = aes_decode(json.encr, secr);
...
// Uploads encrypted CSV data to server for sharing with cloud storage integration
function share_csv() {
    $(document).on("click", "#share_csv", function() {
        const csv = compile_csv(),
...
// Unwraps and decrypts a shared CSV payload
function decode_shared_csv(base64) {
    try {
        const parsed = JSON.parse(atob(strip_quotes(base64))),
            decrypted = parsed.lock ? decrypt_rd_payload(parsed) : false;
        return decrypted ? decrypted.csv : null;
    } catch (e) {
        return null
    }
}
    ...
    api_proxy({
        ...
        if (ping) {
            const cache = ping.br_cache,
                ...
                result = ping.br_result,
                csv_b64 = decode_shared_csv(result.base64);
            if (!csv_b64) {
                systembu_expired();
                return
            }
            const account = b64decode_url(result.account),
                safe_account = escape_html(account),
                title = "CSV Export " + safe_account + " (" + timestr + ")",
                budate = timestr.replace(/\s+/g, "_").replace(/\:/g, "_"),
                cachetime = cache.cache_time,
                expires = (filetime + cachetime) - servertime,
                filename = "bitrequest_csv_export_" + safe_file_part(account) + "_" + budate + ".csv",
                cd = countdown(expires * 1000),
                                        ...
                                        "div": {
                                            ...
                                            "content": [{
                                                ...
                                                "a": {
                                                    ...
                                                    "class": "button icon-download",
                                                    "attr": {
                                                        "href": "data:text/csv;charset=utf-16le;base64," + csv_b64,
                                                        "download": escape_attr(filename),
                                                        "title": escape_attr(filename),
                                                        "data-date": escape_attr(budate),
                                                        "data-lastbackup": escape_attr(filename)
                                                    },
...
function pick_api_proxy() {
    $(document).on("click", "#api_proxy", function() {
        ...
        if (glob_const.phpsupport === true) {
            ...
            if (!objectkey_in_array(proxies, "proxy", url)) {
                proxies.push(proxy_object);
            }
        }
        if (!objectkey_in_array(proxies, "proxy", glob_const.hosted_proxy.proxy)) {
...
function proxy_option_li(options, live, key, value, selected, dfault) {
    const proxy = value.proxy,
        ...
        def = dfault ? " default" : "",
        option = $("<div class='optionwrap" + status + def + "' style='display:none' data-pe='none'><span data-value='" + escape_attr(proxy) + "' data-pe='none'><span class='cstat'>•</span> " + escape_html(proxy) + "</span><div class='opt_icon_box' data-pe='none'>" + tor + "<div class='opt_icon icon-bin' data-pe='none'></div></div>");
...
function submit_api() {
    $(document).on("click", "#apisubmit", function(e) {
        ...
        form.addClass("pass");
        const inputs = form.find("input.ak_input"),
        ...
        form.data("ak_pending", pending.length);
        pending.forEach(function(item) {
            validate_api_key(item.ref, item.val, false);
        });
    });
}
...
// Decrements the pending counter on the apikey form (if present) and returns true
function ak_finalize_check(form) {
    if (!form || !form.length) return true
    const remaining = form.data("ak_pending");
    if (typeof remaining !== "number") return true // single-caller path; finalize immediately
    const next = remaining - 1;
    if (next <= 0) {
        form.removeData("ak_pending");
        return true
    }
    form.data("ak_pending", next);
    return false
}
...
function api_fail(ref, val) {
    ...
    notify(msg);
    ak_finalize_check(form); // decrement pending count so successes can still finalize
}
...
function update_api_attr(thisref, thisvalue, last_input) {
    ...
    if (form && form.hasClass("pass")) {
        ...
        const is_last_done = ak_finalize_check(form);
        if (last_input || is_last_done) {
...
function compile_teaminvite() {
    const jsonfile = {},
        skip_keys = [
            "bitrequest_symbols", "bitrequest_changes", "bitrequest_erc20tokens_init",
            "bitrequest_erc20tokens", "bitrequest_editurl", "bitrequest_recent_requests",
            "bitrequest_backupfile_id", "bitrequest_appstore_dialog", "bitrequest_init",
            "bitrequest_eth_l2_contracts", "bitrequest_k", "bitrequest_awl", "bitrequest_dat", "bitrequest_rt", "bitrequest_tp", "bitrequest_bpdat",
            "bitrequest_requests", "bitrequest_archive", "sameseed"
        ];
    for (let key in localStorage) {
        const val = localStorage.getItem(key);
        if (val !== null && !skip_keys.includes(key)) {
            let parsed;
            try {
    ...
    return encode_rd_payload(adjusted);
}
            ...
            api_proxy({
                ...
                if (ro) {
                    const proxy = c_proxy(),
                        ...
                        app_root = glob_const.approot;
                    shorten_url(title, app_root + "?ro=" + data, app_root + "assets/img/icons/apple-touch-icon.png", true, url_hash);
                }
            }).fail(function(xhr, stat, err) {
                closeloader();
            });
...
function share_teaminvite_qr() {
    $(document).on("click", "#qr_invite", function() {
        const qr_container = $("#ti_qr_container"),
            ...
            result = qr_loaded || confirm(tl("showqr") + "?");
        if (result) {
            ...
            api_proxy({
                ...
                if (ro) {
                    const proxy = c_proxy(),
                        ...
                        app_root = glob_const.approot;
                    shorten_url("qr", app_root + "?ro=" + data, app_root + "assets/img/icons/apple-touch-icon.png", true, url_hash);
                }
            }).fail(function(xhr, stat, err) {
                closeloader();
            });
...
function toggle_ti_qr(url) {
    const qr_container = $("#ti_qr_container"),
        ti_intro = $("#ti_intro");
    if (!qr_container.hasClass("ti_qr")) {
        $("#qrcode").qrcode(url);
...
function install_teaminvite(jo, bu_filename, iid) {
    const jsonobject = (jo.lock) ? decrypt_rd_payload(jo) : jo;
    if (!jsonobject) {
        popnotify("error", tl("error"));
        return
    }
    ...
    const lastrestore = tl("lastrestore") + "<br/><span class='icon-folder-open'>" + tl("teaminvite") + " " + now_display_stamp() + "</span>";
    set_setting("restore", {
...
// Formats a "now" timestamp safe for filenames (replaces spaces and colons with underscores).
function now_filename_stamp() {
    return new Date(now_utc()).toLocaleString(langcode).replace(/\s+/g, "_").replace(/:/g, "_");
}

// Formats a "now" timestamp for display where only spaces need replacing (preserves colons).
function now_display_stamp() {
    return new Date(now_utc()).toLocaleString(langcode).replace(/\s+/g, "_");
}