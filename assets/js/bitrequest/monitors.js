...
// Provider-keyed handlers. Looked up via api_data.name.
const CRYPTO_API_DISPATCH_BY_PROVIDER = {
    "mempool.space": (rd, ad, rdo) => mempoolspace_rpc_init(rd, ad, rdo, false),
    "blockchain.info": current_block_height,
    "blockcypher": process_blockcypher_transactions,
    "etherscan": (rd, ad, rdo) => scan_layer2_transactions(rd, ad, rdo, null, 1),
    "alchemy": initialize_alchemy_scan,
    "ethplorer": process_ethereum_transactions,
    "blockchair": blockchair_fetch,
    "dash.org": process_dash_transactions
};

// Coin-specific scanners. Looked up via rd.payment.
const CRYPTO_API_DISPATCH_BY_PAYMENT = {
    "nimiq": process_nimiq_transactions,
    "kaspa": initialize_kaspa_scan,
    "monero": init_monero_scan
};

// Direct-RPC dispatch (no third-party API in api_data.api). Keyed by payment type.
const BLOCKCHAIN_RPC_DISPATCH = {
    "ethereum": infura_txd_rpc,
    "nano": nano_rpc,
    "monero": init_monero_scan
    // BTC-family handled via is_btchain() check below
};
...
function route_blockchain_rpc(rd, api_data, rdo) {
    ...
    // ERC20 tokens take the ethereum RPC path
    if (rd.erc20 === true) {
        BLOCKCHAIN_RPC_DISPATCH.ethereum(rd, api_data, rdo);
        return
    }
    const handler = BLOCKCHAIN_RPC_DISPATCH[rd.payment];
    if (handler) {
        handler(rd, api_data, rdo);
        return
    }
...
function process_scan_results(rd, api_data, rdo, tx_details, l2) {
    ...
    if (tx_details) {
        ...
        if (src === "tx_polling" || src === "l2_polling") {
            if (over_budget(rdo, "polling")) return // prevent overflow
...
function handle_api_failure(rd, rdo, error_obj, api_data, l2) {
    ...
    const request_id = rd.requestid,
        payment = rd.payment;
    if (l2) {
        const next_l2_api = get_next_layer2_api(payment, api_data, request_id, l2, rdo);
        if (next_l2_api) {
            // Scan eth layer 2
            if (src === "requests") {
        ...
        const next_rpc = get_next_rpc(payment, api_data, request_id, null, rdo);
        if (next_rpc) {
...
// Shared internal: given an endpoint list and the current entry, returns the
// next entry (with wraparound) that hasn't already been tried for this request,
// or false if every entry is exhausted.
function next_unused_endpoint(list, match_field, match_value, cache_key) {
    const list_length = list.length;
    if (!list_length) return false
    const current_index = list.findIndex(option => option[match_field] === match_value),
        next = list[(current_index + 1) % list_length];
    if (glob_let.rpc_attempts[cache_key(next)] !== true) {
        return next;
    }
    return false
}
...
function get_next_layer2_api(this_payment, api_data, request_id, l2, rdo) {
    if (over_budget(rdo, "l2")) return false // prevent overflow
    const api_settings = q_obj(get_coinsettings(this_payment), "layer2.options." + l2 + ".apis");
    if (api_settings) {
        const api_list = api_settings.apis;
        if (api_list) {
            const rq_id = request_id || "";
            return next_unused_endpoint(api_list, "name", api_data.name, next => sha_sub(rq_id + next.url + l2, 15));
        }
    }
    return false
}
...
// rdo is optional — the Monero setup path (handle_xmr_rpc_fails, monero_setup_fail)
// calls this before an rdo exists, in which case we fall back to the
// request-scoped budget.
function get_next_rpc(this_payment, api_data, request_id, l2, rdo) {
    if (rdo ? over_budget(rdo, "rpc") : over_budget_request("rpc")) return false // prevent overflow
    const api_settings = cs_node(this_payment, "apis", true);
    if (api_settings) {
        const api_list = api_settings.apis,
            rpc_list = api_settings.options,
            combined_list = (rpc_list && rpc_list.length) ? [...api_list, ...rpc_list] : api_list;
        if (combined_list) {
            const rq_id = request_id || "",
                l2_prefix = l2 || "";
            return next_unused_endpoint(combined_list, "url", api_data.url, next => sha_sub(rq_id + next.url + l2_prefix, 15));
        }
    }
    return false
}
...
function display_api_source(current_list, api_data) {
    ...
    current_list.data("source", api_source).find(".api_source").html("<span class='src_txt' title='" + escape_attr(api_url) + "'>" + tl("source") + ": " + truncate_middle(api_source, 15, 18, 41) + "</span><span class='icon-wifi-off'></span><span class='icon-connection'></span>");
}
...
function wrap_historic_data(data) {
    return "<div class='historic_meta'>" + escape_html(data).split("\n").join("<br/>") + "</div>";
}
...
// Shared .fail handler for historic-rates API chains. Both fetch_fiat_rates
function historic_fail_chain(opts) {
    // opts: { api_list, current_api, retry_self, retry_with_next, fail_label, rdo }
    return function(xhr, stat, err) {
        const try_next_proxy = function() {
            if (get_next_proxy()) {
                opts.retry_self();
                return true
            }
        ...
        if (is_proxy_fail(this.url) && try_next_proxy()) {
            return
        }
        const next_api = try_next_api(opts.api_list, opts.current_api);
        if (next_api) {
            opts.retry_with_next(next_api);
            return
        }
        if (try_next_proxy()) {
            return
        }
        const error = xhr || stat || err;
        fail_dialogs(opts.fail_label, {
            error
        });
        finalize_request_state(opts.rdo);
    };
}
...
function fetch_fiat_rates(rd, rdo, api_list, fiat_api) {
    ...
    if (fiat_currency) {
        ...
        api_proxy({
            ...
            finalize_request_state(rdo);
        }).fail(historic_fail_chain({
            api_list,
            "current_api": fiat_api,
            "retry_self": () => fetch_fiat_rates(rd, rdo, api_list, fiat_api),
            "retry_with_next": (next_api) => fetch_fiat_rates(rd, rdo, api_list, next_api),
            "fail_label": fiat_api,
            rdo
        }));
        return
...
// Per-provider historic-rate API config. Each entry owns:
const HISTORIC_CRYPTO_API = {
    "coincodex": {
        "coin_id": (rd) => rd.currencysymbol,
        "payload": (coin_id, start, end) => get_payload_historic_coincodex(coin_id, start, end),
        "extract": (api_result) => api_result ? Object.values(api_result)[0] : null,
        "parse": get_historic_object_coincodex,
        "proxy": true
    },
    "coindesk": {
        "coin_id": (rd) => rd.currencysymbol, // historical fallthrough — preserved from original
        "payload": (coin_id, start, end) => get_payload_historic_coindesk(coin_id, start, end),
        "extract": (api_result) => api_result?.Data,
        "parse": get_historic_object_coindesk,
        "proxy": false
    },
    "coinmarketcap": {
        "coin_id": (rd) => rd.cmcid,
        "payload": (coin_id, start, end) => get_payload_historic_coinmarketcap(coin_id, start, end),
        "extract": (api_result) => api_result?.data?.quotes,
        "parse": get_historic_object_coinmarketcap,
        "proxy": true
    },
    "coingecko": {
        "coin_id": (rd) => rd.payment,
        "payload": (coin_id, start, end, rd) => get_payload_historic_coingecko(coin_id, start, end, rd.token_contract),
        "extract": (api_result) => api_result?.prices,
        "parse": get_historic_object_coingecko,
        "proxy": false
    },
    "coinpaprika": {
        "coin_id": (rd) => rd.currencysymbol + "-" + rd.payment,
        "payload": (coin_id, start, end) => get_payload_historic_coinpaprika(coin_id, start, end),
        "extract": (api_result) => api_result,
        "parse": get_historic_object_coinpaprika,
        "proxy": true
    }
};
...
function fetch_crypto_rates(rd, rdo, fiat_api, api_list, api, currency_rate, usd_rate, currency_symbol) {
    ...
    const cfg = HISTORIC_CRYPTO_API[api] || HISTORIC_CRYPTO_API["coinpaprika"], // default to coinpaprika shape
        latest_input = rd.latestinput,
        first_input = rd.firstinput,
        coin_id = cfg.coin_id(rd),
        start_time_sec = parseInt(first_input) / 1000,
        end_time_sec = parseInt(latest_input) / 1000,
        search = cfg.payload(coin_id, start_time_sec, end_time_sec, rd);
    api_proxy({
        "api": api,
        "search": search,
        "cachetime": 86400,
        ...
        api_list,
        "current_api": api,
        "retry_self": () => fetch_crypto_rates(rd, rdo, fiat_api, api_list, api, currency_rate, usd_rate, currency_symbol),
        "retry_with_next": (next_api) => fetch_crypto_rates(rd, rdo, fiat_api, api_list, next_api, currency_rate, usd_rate, currency_symbol),
        "fail_label": api,
        rdo
    }))
}
...
function match_price_timestamps(api, values, price_array, transaction_timestamp) {
    const parse_item = (HISTORIC_CRYPTO_API[api] || HISTORIC_CRYPTO_API["coinpaprika"]).parse;
    for (let i = 0; i < price_array.length; i++) {
        const historic_object = parse_item(price_array[i]);
    ...
    const last_item = price_array[price_array.length - 1],
        last_historic_object = parse_item(last_item);
