...
function fetch_electrum_nodes(currency, node_url, predefined_nodes, custom_nodes) {
    const existing_nodes = predefined_nodes.concat(custom_nodes || []),
    ...
    if (glob_let.ap_id === "apis") {
        ...
        if (is_btchain(currency_name)) {
            ...
            if (rpc_name === "mempool.space") {
                const full_url = complete_url(rpc_url),
                    proxy_data = { // mempoolspace API
                        "api_url": strip_slash(full_url) + "/api/v1/difficulty-adjustment",
                        "proxy": rpc_url.includes(".onion"),
                        "params": {
                            "method": "GET"
                        }
                    }
                if (rpc_url.indexOf("bchexplorer") >= 0) {
                    proxy_data.params.headers = {
                        "User-Agent": glob_const.bchexplorer_agent
                    }; // set custom user_agent
                }
                api_proxy(proxy_data).done(function(e) {
...
function create_rpc_node_element(api_list, is_live, node_id, node_config, is_selected) {
    const status_class = is_live ? " live" : " offline",
        ...
        vendor_string = (vendor) ? "<span class='v'> (" + escape_html(vendor.slice(5)) + ")</span>" : "",
        version = node_config.v,
        version_string = (version) ? "<span class='v'> (" + escape_html(version) + ")</span>" : "",
        display_name = escape_html(setting_sub_address(node_name, stripped_url, node_config.custom)) + vendor_string + version_string,
        ...
        node_element = $("<div class='optionwrap" + status_class + selected_class + default_class + "' style='display:none' data-pe='none' title='" + escape_attr(stripped_url) + "'><span data-value='" + escape_attr(node_url) + "' data-pe='none'>" + node_icon + "<span class='cstat'>•</span> " + display_name + "</span><div class='opt_icon_box' data-pe='none'><div class='opt_icon icon-bin' data-pe='none'></div></div>");
...
function test_xmr_node(rpc_url) {
    ...
    api_proxy({
        "api_url": strip_slash(rpc_url) + "/get_transaction_pool_hashes",
        "proxy": rpc_url.includes(".onion"),
        "params": {
            "method": "POST",
            "headers": {
...
function test_lws_login(rpc_url, address, view_key) {
    const deferred = $.Deferred();
    api_proxy({
        "api_url": strip_slash(rpc_url) + "/login",
        "proxy": false,
        "params": {
            "method": "POST",
            "data": {
...
function test_mempoolspace(input_section, node_config, currency_name) {
    ...
    api_proxy(proxy_data).done(function(response) {
...
function save_rpc_settings(currency_name, node_config, is_new_node) {
    ...
    settings_item.data("selected", node_config).find("p").html(escape_html(display_name));
    if (is_new_node === true) {
