...
// 
// DEPENDENCY: This file requires assets/js/lib/bip39_utils.js for core BIP39/BIP32 functions
...
function restore_seed_verify() {
    $(document).on("click", "#restore_seed", function() {
        const this_node = $(this),
            backup = this_node.hasClass("backup");
        if (glob_let.hasbip && !backup) {
            return false
        }
        ...
        if (is_valid === true) {
            const target_id = this_node.attr("data-seedid"),
                phrase_words = input_phrase.split(" "),
                current_id = get_seedid(phrase_words);
            if (target_id === current_id) {
                const seedsteps = $("#seed_steps");
                if (backup) {
                    const ss_dat = seedsteps.data(),
                        dat = ss_dat.dat,
                        secr = JSON.stringify(phrase_words),
                        json_decrypt = aes_decode(dat, secr);
                    if (json_decrypt) {
                        const filename = ss_dat.filename,
                            pin_hash = get_setting("pinsettings", "pinhash"),
                            seed_data = {
                                "pid": current_id,
                                "pob": btoa(JSON.stringify(phrase_words))
                            },
                            meta = ss_dat.meta,
                            data = {
                                filename,
                        ...
                        if (pin_hash) {
                            ...
                            construct_add_seedobject(data);
                            return
                        }
                        all_pinpanel({
                            "func": construct_add_seedobject,
                            "args": {
                                data
                            }
                        }, true, true);
...
// Construct and add seedobject
function construct_add_seedobject(dat) {
    const data = dat.data || dat,
        pin_hash = data.pin_hash || get_setting("pinsettings", "pinhash"),
        meta = data.meta,
...
// The only producer of the at-rest bitrequest_bpdat shape
function build_seedobject(cb64, seedid, pin_hash) {
    if (!cb64 || !seedid || !pin_hash) return null
    const enc_key = pin_to_encryption_key(pin_hash, seedid);
    return {
        "id": seedid,
        "dat": null,
        "datenc": {
            "id": seedid,
            "dat": aes_enc(JSON.stringify(cb64), enc_key)
        }
    };
}
...
function bip39(dat) {
    ...
    const dialog_data = get_default_object(dat, true),
        ...
        restore_bu = dialog_data?.restore,
        restore_bu_class = (is_restore && restore_bu) ? " backup" : "",
        ui_state = (glob_let.hasbip === true) ? (glob_let.bipv === true) ? "bipsavedbu" : "bipsaved" : "nobip",
        nobip = ui_state === "nobip",
        current_step = nobip ? 1 : (ui_state === "bipsavedbu" ? 2 : 3),
        phrase_class = (nobip || is_restore) ? " showphrase" : " hidephrase",
        ...
        verify_button = is_restore ? "<div id='restore_seed' class='button" + restore_bu_class + "' data-seedid='" + dialog_data.seedid + "'>" + tl("restorebttn") + "</div>" : "<div id='cfbu2' class='button'>" + tl("ivebackeditup") + "</div>",
        ...
        <div id='seed_step2' class='seed_step'>\
            ...
            <div class='ss_content flex'>\
                <div id='phrase_cb' class='ss_content_box" + phrase_class + "'>\
                    <h2 id='showphrase'><span class='icon-eye-blocked eye linkcolor'></span><span class='icon-eye eye linkcolor'></span>" + tl("bip39_seedphrase") + ":</h2>\
                    <p>" + save_prompt + "</p>\
                    <div id='phrasewrap'>\
    ...
    const sd_panel_class = is_restore ? "nobip" : ui_state;
    $("#sd_panel").html(markup).addClass(sd_panel_class);
...
function seed_callback() {
    ...
    if (glob_let.phraseverified === true) {
        ...
        br_set_local("init", updated_init, true);
        glob_let.bipv = true;
        topnotify(tl("seedphraseverified"));
    } else {
...
// Builds the seed-wallet download list markup for a coin
function build_wallet_list(wallet_data) {
    let wallet_list = "";
    if (wallet_data) {
...
function copy_phrase() {
    $(document).on("click", "#copyphrase", function() {
        const phrase = get_mnemonic_phrase(),
            ...
            phrase_label = tl("bip39_seedphrase");
        if (is_valid) {
...
function phrase_info_pu(selected_coin) {
    ...
    const delete_option = selected_coin ? "" : (glob_let.hasbip === true ? "<li class='clearfix'><div id='deletephrase' class='icon-bin'></div></li>" : ""),
    ...
    $.each(glob_config.bitrequest_coin_data, function(i, coin_config) {
        ...
        if (bip32_config.active === true) {
            ...
            let xpub_key,
                ...
                wallet_list = "",
                start_index = 0,
                derived_addresses = keypair_array(seed_hex, new Array(5), start_index, derive_path, bip32_config, master_key, chain_code, coin, undefined, br_format_keys);
            ...
            wallet_list = build_wallet_list(wallet_data);
            const coin_id = coin_symbol + "-" + coin,
                ...
                monero_phrase = (coin === "monero") ? (is_viewonly()) ? false :
...
function list_compatible_wallets(coin) {
    ...
    $.each(glob_config.bitrequest_coin_data, function(i, coin_config) {
        ...
        if (bip32_config.active === true) {
            const wallet_list = build_wallet_list(wallet_data),
                coin_id = coin_symbol + "-" + coin,
                coin_icon_src = c_icons(coin_id),
                icon_element = $("<img src='" + coin_icon_src + "' data-class='pd_" + coin + "'/>"),
...
function derive_address_batch(trigger_element, is_prev) {
    ...
    if (bip39_const.c_derive[coin]) {
        ...
        const bip32_config = path_data.bip32,
            ...
            derived_addresses = keypair_array(key_data.seed, new Array(batch_size), start_index, derive_path, bip32_config, master_key, chain_code, coin, version, br_format_keys);
        address_list.html("");
        $.each(derived_addresses, function(i, address_data) {
            const current_index = start_index + i,
