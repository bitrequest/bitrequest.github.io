...
// ============================================================
// Socket dispatch — per-coin handlers, registered in SOCKET_HANDLERS below.
// To add a coin: write a pick_<coin>_socket function and add it to the table.
// Each handler receives (socket_node, wallet_address, retry) and selects the
// concrete websocket/polling routine. Order of branches must match the
// original if/else cascade — verified by code review, not by tests.
// ============================================================

// Common node-name lookup for BTC-family coins (BTC, LTC, DOGE, BCH).
// Returns the matching handler, or undefined to let the caller fall back.
const BTC_FAMILY_BY_NODE = {
    "blockcypher wss": blockcypher_websocket,
    "blockcypher ws": blockcypherws,
};

// For BTC/LTC/DOGE/BCH: mempool.space or "default=false" → mempoolspace.
function pick_mempoolspace_first(socket_node, wallet_address) {
...
function pick_bitcoin_socket(socket_node, wallet_address, retry) {
    if (wallet_address === "lnurl") {
        // lightning only — fall through to lnd block below
    } else {
        const node_name = socket_node.name;
        if (!pick_mempoolspace_first(socket_node, wallet_address)) {
            const fn = BTC_FAMILY_BY_NODE[node_name];
            if (fn) {
                fn(socket_node, wallet_address);
    ...
    if (helper.lnd_status && !retry) {
        lightning_socket(helper.lnd);
    }
}

function pick_btc_family_socket(socket_node, wallet_address, sochain_label, default_fn) {
    if (pick_mempoolspace_first(socket_node, wallet_address)) return
    const node_name = socket_node.name,
        fn = BTC_FAMILY_BY_NODE[node_name];
    if (fn) {
        fn(socket_node, wallet_address);
        return
    }
    if (node_name === "sochain api") {
        sochain_socket(socket_node, wallet_address, sochain_label);
        return
    }
    default_fn(socket_node, wallet_address);
}

function pick_litecoin_socket(socket_node, wallet_address) {
    pick_btc_family_socket(socket_node, wallet_address, "LTC", blockcypher_websocket);
}

function pick_dogecoin_socket(socket_node, wallet_address) {
    pick_btc_family_socket(socket_node, wallet_address, "DOGE", blockcypher_websocket);
}

function pick_dash_socket(socket_node, wallet_address) {
    const node_name = socket_node.name;
    ...
    const fn = BTC_FAMILY_BY_NODE[node_name];
    if (fn) fn(socket_node, wallet_address);
    // else: no default, original code returned silently
}

function pick_bch_socket(socket_node, wallet_address) {
    if (pick_mempoolspace_first(socket_node, wallet_address)) return
    // original code routes "blockchain.info websocket" AND the fallback to the same fn
    blockchain_bch_socket(socket_node, wallet_address);
}

function pick_ethereum_socket(socket_node, wallet_address, retry) {
...
// Top-level table: payment_type → handler.
// Handlers receive (socket_node, wallet_address, retry).
const SOCKET_HANDLERS = {
    "monero": (sn, addr, retry) => init_xmr_polling(null, retry),
    "ethereum": pick_ethereum_socket,
    "bitcoin": pick_bitcoin_socket,
    "litecoin": pick_litecoin_socket,
    "dogecoin": pick_dogecoin_socket,
    "dash": pick_dash_socket,
    "bitcoin-cash": pick_bch_socket,
    "nano": (sn, addr) => nano_socket(sn, addr),
    "nimiq": () => start_address_monitor(5000),
    "kaspa": () => start_address_monitor(5000)
};

// Establishes WebSocket connections for cryptocurrency payment monitoring
// based on payment type and node configuration. Dispatches via SOCKET_HANDLERS.
function init_socket(socket_node, wallet_address, retry, foreground) {
    if (glob_const.offline || !wallet_address) {
        br_offline();
        return
    }
...
// Only reached for closes we did not initiate (close_socket replaces onclose),
// so treat it like a failure and try the next node / poll fallback.
function handle_socket_close(socket_node, socket_id, wallet_address, is_layer2) {
    console.log("Disconnected from " + socket_node.url);
    if (is_openrequest()) {
        handle_socket_fails(socket_node, wallet_address || socket_id, wallet_address ? socket_id : undefined, is_layer2);
        return
    }
...
function try_next_socket(current_node, is_layer2) {
    ...
    if (over_budget_request("socket")) return false // prevent overflow
    const current_url = current_node.url,
        ...
        available_nodes = socket_config.options ? socket_config.apis.concat(socket_config.options) : socket_config.apis;
...
// Wraps the shared subscribe/ping/close/error skeleton around per-API logic.
function build_socket(socket_node, wallet_address, opts) {
    if (glob_let.sockets[wallet_address]) return
    const socket = glob_let.sockets[wallet_address] = new WebSocket(opts.endpoint),
        subscribe_payload = JSON.stringify(opts.subscribe),
        ping_payload = opts.ping ? JSON.stringify(opts.ping) : subscribe_payload,
        ping_interval = opts.ping_interval || 55000;
    socket.onopen = function(e) {
        socket_info(socket_node, true);
        socket.send(subscribe_payload);
        set_ping(wallet_address, function() {
...
function lightning_socket(lnd, foreground) {
        ...
        try {
            ...
            if (socket_data.pid === payment_id) {
                if (socket_data.status === "pending" && socket_data.bolt11) {
                    ...
                    set_ping(socket_data.hash, function() {
                        lnd_poll_invoice(proxy_url, proxy_key, invoice_mode, socket_data, payment_id, node_id);
                    }, 5000);
                }
    ...
    socket.onclose = function(e) {
        ...
        const unexpected = glob_let.sockets[payment_id] === socket; // deliberate closes delete this first
        delete glob_let.sockets[payment_id];
        if (unexpected && is_openrequest()) {
            poll_lightning_status(proxy_url, proxy_key, payment_id, node_id, invoice_mode);
            return
        }
        close_socket(payment_id);
    ...
    socket.onerror = function(e) {
        console.error("Lightning socket error"); // onclose always follows and handles the fallback
    };
        ...
        $.ajax({
            ...
            if (response.pid == payment_id) {
                if (response.status === "pending" && response.bolt11) {
                    ...
                    set_ping(response.hash, function() {
                        lnd_poll_invoice(proxy_host, proxy_key, invoice_mode, response, payment_id, node_id);
                    }, 5000);
                    return
                }
...
function blockcypher_websocket(socket_node, wallet_address) {
    build_socket(socket_node, wallet_address, {
        "endpoint": socket_node.url + request.currencysymbol + "/main",
        "subscribe": {
        ...
        "on_message": function(msg_data) {
            if (msg_data.event === "pong") return null
            if (!msg_data.hash) return null
            const setconfirmations = request.set_confirmations || 0,
                tx_details = blockcypher_poll_data(msg_data, setconfirmations, request.currencysymbol, wallet_address);
...
function blockchain_btc_socket(socket_node, wallet_address) {
    build_socket(socket_node, wallet_address, {
        ...
        "on_message": function(msg_data) {
            const tx_data = msg_data.x;
            if (!tx_data || !tx_data.hash) return null
            const setconfirmations = request.set_confirmations || 0;
            return blockchain_ws_data(tx_data, setconfirmations, request.currencysymbol, wallet_address);
        }
    });
}
...
function blockchain_bch_socket(socket_node, wallet_address) {
    ...
    build_socket(socket_node, wallet_address, {
        ...
        "on_message": function(msg_data) {
            ...
            return blockchain_ws_data(tx_data, setconfirmations, request.currencysymbol, wallet_address, legacy_format);
        }
    });
}
...
function mempoolspace_btc_socket(socket_node, wallet_address) {
    build_socket(socket_node, wallet_address, {
        ...
        "on_message": function(msg_data) {
            const tx_batch = msg_data["address-transactions"];
            if (!tx_batch) return null
            const tx_data = tx_batch[0];
            if (!tx_data || !tx_data.txid) return null
            const setconfirmations = request.set_confirmations || 0;
            return mempoolspace_ws_data(tx_data, setconfirmations, request.currencysymbol, wallet_address);
        }
    });
}

// Establishes WebSocket connection to chain.so for Bitcoin, Litecoin, Dogecoin
// address monitoring with transaction validation.
//
// Intentionally does NOT use build_socket(): chain.so's WS protocol is a
// change-notifier, not a tx-delivery channel. The socket only signals
// "address has new activity"; the actual tx data is fetched by polling
// (start_address_monitor) after the socket fires. The socket stays open
// across notifications, and the 20s keepalive matches what chain.so's
// connection timeout requires.
...
function init_eth_sockets(params, token_contracts) {
    ...
    if (payment_type === "ethereum") {
        if (socket_node.name === glob_const.main_alchemy_socket || socket_node.name === "alchemy") {
...
function alchemy_eth_websocket(socket_node, wallet_address) {
    ...
    build_socket(socket_node, wallet_address, {
        ...
        "on_message": function(msg_data) {
            const tx_data = q_obj(msg_data, "params.result");
            if (!tx_data || !tx_data.hash || !addr_eq(tx_data.to, wallet_address)) return null
            const setconfirmations = request.set_confirmations || 0;
            return infura_block_data(tx_data, setconfirmations, request.currencysymbol);
        }
    });
}
...
function web3_eth_websocket(socket_node, wallet_address) {
    ...
    socket.onclose = function(e) {
        handle_socket_close(socket_node, socket_id, wallet_address);
    };
    socket.onerror = function(e) {
...
function web3_erc20_websocket(socket_node, wallet_address, contract_address, socket_id) {
    ...
    socket.onclose = function(e) {
        ...
        handle_socket_close(socket_node, socket_id, wallet_address, network_type);
    };
    socket.onerror = function(e) {
        handle_socket_fails(socket_node, wallet_address, socket_id, network_type);
    };
...
function nano_socket(socket_node, wallet_address) {
    ...
    build_socket(socket_node, wallet_address, {
        ...
        "on_message": function(msg_data) {
            const tx_data = (msg_data.message) ? msg_data.message : (msg_data.account) ? msg_data : null;
            if (!tx_data) return null
            if (tx_data.account === wallet_address) return null // block outgoing transactions
            if (!tx_data.hash) return null
            const reciever = tx_data?.block?.link_as_account;
            if (reciever && !addr_eq(reciever, normalized_address)) return null // block non matching transactions
            const tx_details = nano_scan_data(tx_data),
                tx_time = tx_details.transactiontime,
                time_delta = Math.abs(tx_time - now_utc());
            return (time_delta < 60000) ? tx_details : null // filter transactions longer then a minute ago
        }
    });
}