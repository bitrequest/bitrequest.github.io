...
function setlangcode() {
    ...
    if (settingcache) { // get saved language
        ...
        try {
            parse_cache = JSON.parse(settingcache);
        } catch (e) {}
        const lang_settings = parse_cache && parse_cache[2];
        if (lang_settings) {
            const setlang = lang_settings.selected,
                sl_length = typeof setlang === "string" ? setlang.length : 0;
...
// Looks up a translation string and fills {placeholder} tokens from data.
// Unknown placeholders are left intact; missing keys return undefined so
// tl_resolve can fall back to English.
function lang_tl(translations, id, data) {
