...
function blockchair_eth_scan(rd, api_data, rdo) {
    ...
    run_address_scan(rd, api_data, rdo, {
        ...
        "match": (parsed_tx, _tx, _rd, rdo) => parsed_tx.ccval && parsed_tx.transactiontime > rdo.request_timestamp && addr_eq(parsed_tx.recipient, wallet_address)
    });
}
...
function blockchair_erc20_scan(rd, api_data, rdo) {
    ...
    run_address_scan(rd, api_data, rdo, {
        ...
        "match": (parsed_tx, _tx, _rd, rdo) =>
            ...
            addr_eq(parsed_tx.recipient, wallet_address) === true &&
            str_eq(parsed_tx.token_symbol, currency_symbol) === true &&
            parsed_tx.ccval
    });
}
...
function process_alchemy_transactions(rd, api_data, rdo, ctract, block_height) {
    ...
    run_address_scan(rd, api_data, rdo, {
        ...
        "match": (parsed_tx, tx, rd, rdo) => {
            ...
            return addr_eq(tx.to, rd.address) && parsed_tx.transactiontime > adjusted_timestamp && parsed_tx.ccval && poll_match;
        },
        "display_on_match": true,
...
function monero_lws_login(rd, api_data, rdo) {
    ...
    const address = vk_object.account || rd.address,
        data = {
            address,
            view_key,
            "create_account": true,
...
function mempoolspace_rpc_blockheight(rd, api_data, rdo, rpc) {
    ...
    if (base_url.indexOf("bchexplorer") >= 0) {
        proxy_data.params.headers = {
            "User-Agent": glob_const.bchexplorer_agent
        }; // set custom user_agent
    }
    api_proxy(proxy_data).done(function(response) { // get latest blockheight
...
function blockchain_ws_data(data, setconfirmations, ccsymbol, address, legacy) {
    function process_value(output, target_addr) {
        ...
        return addr_eq(output_addr, target_addr) ||
            addr_eq(output_addr, legacy) ?
            output.value || 0 : 0;
    }
...
// Matches a mempool.space output against the target address
function mempool_output_value(output, target_addr) {
    return addr_eq(output.scriptpubkey_address, target_addr) ? output.value || 0 : 0;
}
...
function mempoolspace_ws_data(data, setconfirmations, ccsymbol, address) {
    const total_output = calculate_total_outputs(data.vout, address, mempool_output_value),
...
function mempoolspace_scan_data(data, setconfirmations, ccsymbol, address, latest_block) {
    const status = data.status,
        ...
        total_output = calculate_total_outputs(data.vout, address, mempool_output_value),
...
function electrum_scan_data(data, setconfirmations, ccsymbol, script_pub, latest_block, tx_hash) {
    const outputs = data.outputs,
        ...
        time_correction = height > 0 ? timestamp : now_utc(),
...
function insight_scan_data(data, setconfirmations, address) {
    ...
    function process_value(output) {
        ...
        return str_eq(script_hex, output_script) ? parseFloat(output.value) || 0 : 0;
    }
...
function blockchaininfo_scan_data(data, setconfirmations, ccsymbol, address, latest_block) {
    ...
    function process_output_value(output, target_addr) {
        return addr_eq(output.address, target_addr) ? Math.abs(output.value) : 0;
    }
...
function blockchair_scan_data(data, setconfirmations, ccsymbol, address, latest_block) {
    ...
    function process_value(output, target_addr) {
        const output_value = output.value;
        return addr_eq(output.recipient, target_addr) ? Math.abs(output_value) || 0 : 0;
    }
