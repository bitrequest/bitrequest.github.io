...
function set_symbols() {
    ...
    api_proxy({
        ...
        const error_msg = "<h2 class='icon-bin'>" + tl("apicallfailed") + "</h2><p class='doselect'>" + stat + "<br/>" + tl("apididnotrespond") + "<br/><br/><span id='proxy_dialog' class='ref'>" + tl("tryotherproxy") + "</span></p>";
        popdialog(error_msg, "canceldialog");
    })
}
...
function enterapp(pin_input) {
    ...
    if (String(hashed_pin) === String(stored_pin)) {
        if (is_global) {
            ...
            if (pin_callback) {
                const args = pin_callback?.args;
                if (args) {
                    args.pin_hash = hashed_pin;
                };
                pin_callback.func(args);
            } else {
                br_set_local("locktime", timestamp);
            }
            play_audio("waterdrop");
    ...
    if (is_valid === true) {
        ...
        if (is_extended_pub) {
            if (cxpub(payment)) {
                ...
                popnotify("error", "invalid " + escape_html(payment) + " address");
            }
            return
        }
        return
...
function handle_viewkey(result, payment) {
    const is_valid = (result.length === 64) ? is_hex(result) : false;
    if (is_valid === true) {
    ...
    popnotify("error", "invalid " + escape_html(payment) + " viewkey");
}
...
function countdown(timestamp) {
    ...
    const days = Math.floor(time_secs / 86400);
    time_secs -= days * 86400;
    const hours = Math.floor(time_secs / 3600) % 24;
    time_secs -= hours * 3600;
    const minutes = Math.floor(time_secs / 60) % 60;
    time_secs -= minutes * 60;
    const seconds = Math.round(time_secs % 60),
        time_parts = {
            days,
            hours,
            minutes,
        ...
        "ul": {
            ...
            "content": [{
                ...
                },
                {
                    "li": {
                        "id": "init_lnd",
                        "content": "<img src='assets/img/logos/btc-lnd.png'/> Lightning <span class='icon-power'/>"
                    }
                }
            ]
        }
...
function newrequest() {
    $(document).on("click", ".newrequest", function() {
        const request_btn = $(this),
            ...
            address = addr_data.address,
            ccsymbol = addr_data.ccsymbol,
            title = request_btn.attr("title"),
            seedid = addr_data.seedid;
        if (seedid) {
        ...
        canceloptions();
        finishtx_function(currency, address, null, title);
    });
}
...
function show_vk_cb(view_key) {
    ...
    const view_key_text = key_data.svk ? "<br/><strong style='color:#8d8d8d'>" + tl("secretviewkey") + "</strong> <span class='adbox adboxl select' data-type='Viewkey'>" + escape_html(key_data.svk) + "</span><br/>" : "",
        spend_key_text = key_data.ssk ? "<br/><strong style='color:#8d8d8d'>" + tl("secretspendkey") + "</strong> <span class='adbox adboxl select' data-type='Spendkey'>" + escape_html(key_data.ssk) + "</span>" : ""
    ...
    const form_data = {
        ...
        <div class='formbox' id='erc20formbox'>\
            ...
            <form id='addressform' class='popform'>\
                ...
                <input type='text' class='addresslabel' value='" + escape_attr(eth_label_value) + "' placeholder='" + tl("label") + "'/>\
                <div id='pk_confirm' class='noselect'>\
...
function submit_request_description() {
    $(document).on("click", "#edit_request_formbox input.submit", function(event) {
        const submit_btn = $(this),
            requestid = submit_btn.attr("data-requestid"),
            title_input = submit_btn.prev("input").val(),
            requesttitle = title_input || "empty";
        if (title_input) {
            update_request({
                requestid,
                requesttitle
...
function share_receipt() {
    $(document).on("click", "#share_receipt", function() {
        const share_btn = $(this),
            ...
            filename = "bitrequest_receipt_" + request_id + ".pdf",
...
function lnd_lookup_invoice(proxy, imp, hash, node_id, peer_id, password, spark_request_id) {
    ...
    $.ajax(request_data).done(function(response) {
        ...
        if (error) {
            popdialog("<h2 class='icon-blocked'>" + escape_html(error.message) + "</h2>", "canceldialog");
            closeloader();
            return
        }
        const dialog_elems = [{
...
function get_address_warning(dialog_id, wallet_address, dialog_data) {
    const key_type = dialog_data.xpubid ? "Xpub" : "Seed",
        ...
        key_title = (key_type === "Seed") ? tl("bip39_seedphrase") : key_type;
...
function expand_shorturl(input_param) {
    ...
    if (input_param) {
        const proxy_index = input_param.slice(0, 1),
            ...
            proxy_entry = /^\d$/.test(proxy_index) ? glob_const.proxy_list[proxy_index] : null,
            proxy_url = proxy_entry ? proxy_entry.proxy : "",
...
function expand_bitly_url(input_param) {
    ...
    api_proxy({
        ...
        if (parsed_data.error) {
            glob_const.w_loc.href = "https://bit.ly/" + bitly_id;
            return
        }
        if (parsed_data) {
            const long_url = parsed_data.long_url;
            if (long_url && is_own_app_url(long_url)) {
...
// True when a URL points at this app (any known PWA origin or the current one)
function is_own_app_url(url) {
    try {
        const origin = new URL(url).origin;
        return origin === glob_const.w_loc.origin || glob_const.app_origins.includes(origin);
    } catch (e) {
        return false
    }
}
...
function open_blockexplorer_url(url) {
...
function append_request(rd) {
    ...
    unit = uoa.toUpperCase(),
        ...
        (is_incoming ? escape_html(requestname || "") : title_truncated) :
    ...
    const expired_class = is_expired ? " expired" : "",
        ...
        source_html = source ?
        "<span class='src_txt'>" + tl("source") + ": " + escape_html(source) + "</span>" +
        ...
        "<span class='select' data-type='payment ID'>" + escape_html(payment_id) + "</span></li>" : "",
        int_addr_box = xmr_ia ?
        "<li><p class='address'><strong>" + tl("integratedaddress") +
        ":</strong> <span class='requestaddress select'>" + escape_html(xmr_ia) + "</span></p></li>" : "",
        ln_icon = is_lightning_tx ? " <span class='icon-power'></span>" : "",
        ln_logo = "<img src='assets/img/logos/btc-lnd.png' class='cmc_icon'>" +
        "<img src='assets/img/logos/btc-lnd.png' class='cmc_icon'>",
        ...
        bolt_card = boltcard ? "<img src='assets/img/boltcard.png' class='boltcard' title='paid with the boltcard'>" : "",
