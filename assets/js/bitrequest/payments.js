    ...
    $(document).on("mousemove touchmove", "#payment", function(e) {
        ...
        if (Math.abs(drag_distance) > 3) { // margin to activate swipe
            ...
            const distance_ratio = drag_distance / dialog_height,
                opacity = 1 - Math.abs(distance_ratio);
...
function load_request(pass) {
    ...
    // Per-request retry budgets attach to `request` once it's created below;
    // any prior request's attempts were already cleared when that request closed.
    ...
    if (glob_let.symbolcache) {
        ...
        const contactform = exists(url_params.contactform);
...
function continue_request(contracts) {
    ...
    const current_confirmations = q_obj(coin_settings, "confirmations.selected"),
        ...
        page_name = request_name ? tl(request_name === "Bitrequest" ? "sharetitlename_own" : "sharetitlename_sent", {
...
function monero_setup(api_data) {
    if (api_data) {
        ...
        xmr_rpc(api_data, "get_last_block_header").done(function(e) {
    ...
    $.ajax({
        ...
        if (response_error) {
            ...
            if (request.isrequest) {
                ...
                notify(escape_html(error_message));
                $("#rq_errlog").append("<span class='rq_err'>" + escape_html(error_message) + "</span>");
            }
        }
        proceed_pf();
    }).fail(function(xhr, status, error) {
...
function no_xrate_result(api, error) {
    closeloader();
    cancel_paymentdialog();
    fail_dialogs(api, {
        error
...
function render_currencypool(data, crypto_rate, crypto_api, fiat_api, cache_age_crypto, cache_age_fiat) {
    ...
    $.each(data, function(currency, rate) {
        const parsed_rate = (rate / current_rate) / 1,
            ...
            currency_name = glob_let.symbolcache[currency.toUpperCase()],
            rate_html = "<div data-currency='" + currency + "' data-value='' data-xrate='" + rate + "' class='cpool' data-currencyname='" + currency_name + "'><span class='ratesspan" + rate_span_class + "'>" + request.uoa + "_" + currency + ": " + parsed_rate.toFixed(2) + "</span></div>";
    ...
    const lightning_info = helper.lnd,
        proxy_icon = lightning_info && lightning_info.proxy ? "<span class='icon-sphere' title='" + escape_attr(lightning_info.proxy_host) + "'></span>" : "",
        ...
        lightning_node_name = request.isrequest === true ? "" : ": <span id='lnd_nodeinfo_trigger' title='" + escape_attr(node_name) + "'>" + escape_html(truncate_middle(node_name)) + "</span>",
        lightning_node_info = lightning_info ? "<br/><span id='current_lndnode'><img src='" + c_icons(lightning_info.imp) + "' class='lnd_icon' title='" + escape_attr(lightning_info.imp) + "'/> Lightning node" + lightning_node_name + lightning_status_icon + proxy_icon + "</span>" : "";
...
function get_payment(ccrateeuro, ccapi) {
    ...
    const exchange_rate = get_xrate(request.uoa),
        ...
        label_html = has_label ? "<span id='labelbttn' class='linkcolor'>" + escape_html(label_text) + "</span>" : "", // check if label is set
        ...
        fallback_label = has_label ? " (" + escape_html(label_text) + ")" : "",
        fallback_html = (request.payment === "bitcoin" && !helper.lnd_only) ? "<div id='fallback_address'>Fallback address:<br/><span id='fb_addr'>" + escape_html(request.address) + fallback_label + "</span> " + switch_panel(false, " global") + "</div>" : "",
        share_form = "\
            <div id='shareformbox'>\
...
function update_exchange_rates_text(nextcurrency, newccrate) {
    $("#paymentdialog .cpool").each(function() {
        const current_node = $(this),
            ...
            currency = current_node.attr("data-currency"),
            new_rate = current_node_rate / newccrate,
            is_cc_pool = current_node.hasClass("ccpool"),
            parsed_text = is_cc_pool ? currency + "_" + nextcurrency : nextcurrency + "_" + currency,
...
function switch_currency() {
    $(document).on("click", "#paymentdialogbox #pickcurrency", function() {
        const current_node = $(this),
            ...
            newccvalue = newccnode.attr("data-value").replace(/\.00$/, ""),
            ...
            newccvaluevar = this_iszero ? "" : newccvalue,
            newccvalueplaceholder = this_iszero ? glob_const.zeroplaceholder : newccvalue,
...
function validate_steps() {
    ...
    function full_normalize(value, remove_trailing_dot = false) {
        ...
        let result = int_part + (dec_part.length ? "." + dec_part : (ends_with_dot ? "." : ""));
        return result;
    }
...
function pick_address_from_dialog() {
    $(document).on("click", "#addresslock #pending_pick_address", function(e) {
        ...
        const address_input = $("#selec_address"),
            thisinputvalue = address_input.val(),
...
function add_address_from_dialog() {
    $(document).on("click", "#addresslock #pending_add_address, #addaddress", function(e) {
        ...
        const form_box = $(this).closest("#addresslock"),
            ...
            viewkey_scanner = glob_let.hascam ? "<div class='qrscanner' data-currency='" + currency + "' data-id='viewkey' title='scan qr-code'><span class='icon-qrcode'></span></div>" : "",
            viewkey_box = currency === "monero" ? "<div class='inputwrap'><input type='text' class='vk_input' value='' placeholder='View key'>" + viewkey_scanner + "</div>" : "",
            derivation_source = dd ? (dd.xpubid ? " from Xpub" : " from seed") : "",
            seed_message = dd ? "<div class='popnotify' style='display:block'><span id='addfromseed' class='address_option linkcolor'>Generate address" + derivation_source + "</span></div>" : "<div class='popnotify'></div>",
            content = $("<div class='formbox form add' id='addressformbox'><h2>" + getcc_icon(cmcid, request.cpid, erc20) + " " + tl("addcoinaddress", {
                currency
...
function share(current_button) {
    if (current_button.hasClass("sbactive")) {
        ...
        const payment = url_params.payment,
            ...
            shared_title = has_data ? tl(request_name_uppercase === "Bitrequest" ? "sharetitlename_own" : "sharetitlename_sent", {
            ...
            share_icon = is_lightning ? glob_const.approot + "assets/img/logos/btc-lnd.png" : glob_const.cmc_icon_loc + currency_id + ".png";
        if (is_ipfs) {
...
function custom_shorten(service, shared_url, shared_title, site_thumb, url_hash) {
    ...
    $.ajax({
        ...
        if (data) {
            if (data.error) {
                notify(escape_html(server + ": " + data.error), 500000, "yes");
                share_request(shared_url, shared_title);
                return
            }
...
function get_saved_shorturl(hash) {
    try {
        ...
        return br_get_session(cache_prefix + "_shorturl_" + hash) || br_get_session("custom_shorturl_" + hash);
    } catch (e) {
        return false
    }
}
...
// Binds a delegated click handler for every entry in share_targets.
function init_share_handlers() {
    const share_targets = {
        "#whatsappshare": {
            "open": "location",
            "url": i => "whatsapp://send?text=" + encodeURIComponent(i.body)
        },
        "#mailto": {
            "open": "location",
            "url": i => "mailto:?subject=" + encodeURIComponent(i.title) + "&body=" + encodeURIComponent(i.body)
        },
        "#gmailshare": {
            "open": "open",
            "url": i => "https://mail.google.com/mail/?view=cm&fs=1&su=" + encodeURIComponent(i.title) + "&body=" + encodeURIComponent(i.body)
        },
        "#telegramshare": {
            "open": "open",
            "url": i => "https://telegram.me/share/url?url=" + encodeURIComponent(i.url) + "&text=" + encodeURIComponent(i.body)
        },
        "#outlookshare": {
            "open": "location",
...
// Clipboard share — kept separate because the sequence is reversed
// (copy first, then share_callback closes the dialog).
function copy_url() {
            ...
            $.each(wallet_list, function(key, wallet_info) {
                ...
                if (platform_url) {
                    const wallet_name = wallet_info.name,
                        ...
                        wallet_icon = is_lightning ? "<img src='assets/img/logos/btc-lnd.png' class='wallet_icon'/>" : get_aws_icon_url(wallet_name),
...
function save_payment_request(direct, lightning_url) {
    ...
    if (is_checkout) {
        const contact_param = url_params.contactform,
            meta_data_object = decode_meta_param(request_meta_hash), // decode meta param if exists
...
// Decodes the checkout meta param; a malformed value must not abort the post-payment flow
function decode_meta_param(meta_hash) {
    if (!meta_hash) return null
    try {
        return JSON.parse(atob(meta_hash));
    } catch (e) {
        console.warn("invalid meta param ignored", e);
        return null
    }
}
