...
function start_transaction_monitor(tx_data, api_data, retry) {
    // Entering the tx-monitoring phase — clear per-request retry budgets so
    // the new phase gets a fresh allowance.
    if (request) request.attempts = {};
...
function start_address_monitor(time_out, api_dat, retry) {
    ...
    const api_data = api_dat || q_obj(helper, "api_info.data");
...
// Builds an api_proxy call for a Monero JSON-RPC method
function xmr_rpc(api_data, method, params) {
    ...
    if (crypto_symbol) {
        ...
        if (tx_data && tx_data.ccval) {
            ...
            const set_confirmations = tx_data.setconfirmations ? parseInt(tx_data.setconfirmations, 10) : 0,
                ...
                is_instant = tx_data.instant_lock; // Dashpay instant_lock (0-conf is handled by confirmations >= set_confirmations, i.e. 0 >= 0)
