        ...
        invoices.reverse().forEach(function(invoice) {
            ...
            if (invoice_description && invoice_description.indexOf("test invoice") === 0) {
                ...
                const invoice_id = invoice.payment_request ? " " + invoice.payment_request.slice(0, 16) :
                    ...
                    bolt_card = invoice_title.indexOf("Boltcard") === 0 ? " <img src='assets/img/boltcard.png' class='boltcard' title='paid with the boltcard'>" : "",
                ...
                invoices_list += "<div class='ivli'><div class='invoice_memo clearfix'>" +
                    "<div class='iv_title' title='" + escape_attr(invoice_title) + "'>" + icon_span + " " + escape_html(truncate_middle(invoice_title, 10, 14, 25)) + bolt_card + "</div>" +
    ...
    $.ajax({
        ...
        if (error) {
            ...
            if (request.isrequest) {
                ...
                const ehtml = escape_html(error_message);
                notify(ehtml);
                $("#rq_errlog").append("<span class='rq_err'>" + ehtml + "</span>");
            }
        }
        const node_metadata = response.mdat;
        if (node_metadata) {
